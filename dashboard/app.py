...
Fixes applied:
  - "undefined" chart titles removed (plotly title kwarg was broken)
  - Tab 2 Forecast: added Model Selection Distribution pie + MAPE Distribution histogram
  - CSS: removed top padding, hidden stHeader, added gradient accent line
  - Auto-pipeline on first boot (_ensure_pipeline)
  - Absolute paths via ROOT / Path(__file__)
...
/* ── Remove top whitespace & hide Streamlit header ── */
...
def _ensure_pipeline() -> None:
    """Auto-generate data + run pipeline if CSV outputs missing (Streamlit Cloud)."""
    sentinel = ROOT / "forecast" / "outputs" / "forecast_qty.csv"
    if sentinel.exists():
        return
    try:
        import main as _main_module
        with st.spinner("⚙️  First-run setup — generating data & running pipeline …"):
            _main_module.run_all()
...
def plotly_layout(**extra) -> dict:
    """Merge PLOTLY_LAYOUT with extra keys safely."""
...
with st.sidebar:
    ...
    g_horizon   = st.select_slider("Forecast Horizon (days)",  options=[30,60,90,120,180], value=90,  key="g_horizon")
    g_test_days = st.select_slider("Evaluation Window (days)", options=[14,21,30,45,60],   value=30,  key="g_test")

    sb_section("📦 Inventory Parameters")

    g_lead_time     = st.slider("Lead Time Override (days)",  min_value=1,  max_value=30, value=14, step=1,  key="g_lt")
    g_service_level = st.slider("Service Level Target (%)",   min_value=85, max_value=99, value=95, step=1,  key="g_sl")
    g_review_period = st.select_slider("Review Period (days)", options=[7,14,30], value=14, key="g_rp")

    sb_section("🚚 Supply Parameters")

    g_safety_buffer = st.slider("Safety Buffer (%)", min_value=0, max_value=30, value=10, step=5, key="g_sb")
    g_moq_override  = st.toggle("Apply Min Order Qty Override", value=False, key="g_moq")
...
with tab1:
    ...
    with col2:
        ...
        if not policy.empty:
            ...
            fig = go.Figure(go.Bar(
                ...
                orientation="h",
                marker=dict(
                    color=wh_val["inventory_value_vnd"],
                    colorscale=[[0,"#1e2736"],[1,"#00d4ff"]], showscale=False,
                ),
                text=wh_val["inventory_value_vnd"].map(lambda x: f"{x:.2f}B"),
                textposition="outside", textfont=dict(size=10),
            ))
            fig.update_layout(**plotly_layout(height=260, xaxis_title="Inventory Value (B VND)", yaxis_title=""))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No policy data available.")
    ...
    with col3:
        ...
        if not policy.empty:
            ...
            fig.update_layout(**plotly_layout(barmode="group", height=260, xaxis_title="ABC Class", yaxis_title="Units (avg)"))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No policy data available.")
    ...
    with col4:
        ...
        if not policy.empty:
            ...
            fig.update_layout(**plotly_layout(height=260, yaxis_title="Stockout Risk (%)", showlegend=False))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No policy data available.")
...
with tab2:
    ...
    cards += kpi_card("Mean Bias",         f"{fk.get('mean_bias',0):+.1f}",              "units/day", "accent2")
    cards += kpi_card("SKU×WH Combos",     str(int(fk.get('n_combinations',0))),         "evaluated", "accent")
    cards += '</div>'
    st.markdown(cards, unsafe_allow_html=True)

    # ── Model Selection Distribution + MAPE Histogram ────────────────────
    ...
    with fc_col1:
        ...
        if not model_sel_df.empty and "best_model" in model_sel_df.columns:
            ...
            total_combos = len(model_sel_df)
            fig.add_annotation(
                text=f"<b>{total_combos}</b><br><span style='font-size:10px'>SKU×WH</span>",
                x=0.5, y=0.5, showarrow=False,
    ...
    # ── Forecast chart (SKU × Warehouse) ─────────────────────────────────
    section("Demand Forecast by SKU × Warehouse")

    fc_f1, fc_f2, fc_f3 = st.columns(3)
    with fc_f1:
        sku_opts = sorted(forecast_df["sku"].unique().tolist()) if not forecast_df.empty else []
        sel_sku  = st.selectbox("SKU", sku_opts, key="fc_sku") if sku_opts else None
    with fc_f2:
        wh_opts2 = sorted(forecast_df["warehouse"].unique().tolist()) if not forecast_df.empty else []
        sel_wh2  = st.selectbox("Warehouse", wh_opts2, key="fc_wh") if wh_opts2 else None
    with fc_f3:
        show_band = st.toggle("Show Confidence Band", value=True, key="fc_band")

    if sel_sku and sel_wh2 and not forecast_df.empty:
        fig = go.Figure()

        # Historical sales
        if not sales_df.empty:
            hist_sub = sales_df[
                (sales_df["sku"] == sel_sku) &
                (sales_df["warehouse"] == sel_wh2)
            ].groupby("date")["sales_qty"].sum().reset_index()
            hist_sub.columns = ["date","qty"]
            if not hist_sub.empty:
                fig.add_trace(go.Scatter(
                    x=hist_sub["date"], y=hist_sub["qty"],
                    name="Historical Sales",
                    line=dict(color="#64748b", width=1.5), mode="lines",
                ))
        else:
            hist_sub = pd.DataFrame()

        # Determine which models to plot
        if g_fc_model == "Auto (Best)":
            models_to_plot = accuracy_df["model"].unique().tolist() if not accuracy_df.empty else list(MODEL_COLORS.keys())
        else:
            models_to_plot = [g_fc_model]
        ...
        for m in models_to_plot:
            ...
            if "model" in fc_sub.columns:
                fc_sub = fc_sub[fc_sub["model"] == m]
            if fc_sub.empty:
                continue
        ...
        # FIX: set title via update_layout separately to avoid "undefined"
        fig.update_layout(title=dict(text=f"{sel_sku} · {sel_wh2}", font=dict(size=13, color="#e2e8f0")))
        st.plotly_chart(fig, use_container_width=True)

    # ── MAPE by ABC class ─────────────────────────────────────────────────
    if not accuracy_df.empty and not policy_raw.empty:
        section("Forecast Accuracy vs ABC Class")
        acc_with_abc = accuracy_df.merge(
            policy_raw[["sku","warehouse","abc_class"]].drop_duplicates(),
            on=["sku","warehouse"], how="left"
        )
        if "abc_class" in acc_with_abc.columns:
            abc_acc = acc_with_abc.groupby("abc_class")["mape"].mean().reset_index()
            fig = go.Figure(go.Bar(
                x=abc_acc["abc_class"], y=abc_acc["mape"],
                marker_color=["#00d4ff","#ffb830","#7fff6b"],
                text=abc_acc["mape"].map(lambda x: f"{x:.1f}%"),
                textposition="outside",
            ))
            fig.update_layout(**plotly_layout(height=260, xaxis_title="ABC Class", yaxis_title="Mean MAPE (%)"))
            st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    with lf1:
        status_filter = st.selectbox("Policy Status", ["All","CRITICAL","REORDER","OK","OVERSTOCK"], key="inv_status")
    with lf2:
        dos_max = st.slider("Max DOS (days)", 1, 60, 60, key="inv_dos")
    with lf3:
        risk_min = st.slider("Min Stockout Risk (%)", 0, 100, 0, key="inv_risk")
    with lf4:
        sort_inv = st.selectbox("Sort Table By", ["stockout_risk","dos","inventory_value_vnd","on_hand_units"], key="inv_sort")
    ...
    if not policy_f.empty:
        policy_f = policy_f[policy_f["dos"] <= dos_max]
        policy_f = policy_f[policy_f["stockout_risk"] * 100 >= risk_min]

    from scipy import stats as scipy_stats

    def _recalc_ss(row, lt_override, sl_override):
        z  = scipy_stats.norm.ppf(sl_override / 100)
        ss = z * row["demand_std"] * np.sqrt(lt_override)
        return max(ss, 0)

    if not policy_f.empty and "demand_std" in policy_f.columns:
        policy_f = policy_f.copy()
        policy_f["safety_stock"]   = policy_f.apply(lambda r: _recalc_ss(r, g_lead_time, g_service_level), axis=1)
        policy_f["reorder_point"]  = policy_f["demand_avg"] * g_lead_time + policy_f["safety_stock"]
        policy_f["stockout_risk"]  = np.where(
            policy_f["on_hand_units"] <= policy_f["reorder_point"],
            np.clip(1 - policy_f["on_hand_units"] / policy_f["reorder_point"].replace(0, np.nan), 0, 1),
            policy_f["stockout_risk"],
        ).clip(0, 1)

    section("Inventory Overview")
    if not policy_f.empty:
        n_crit = int((policy_f["policy_status"] == "CRITICAL").sum())
        n_reorder = int((policy_f["policy_status"] == "REORDER").sum())
        n_ok  = int((policy_f["policy_status"] == "OK").sum())
        n_over = int((policy_f["policy_status"] == "OVERSTOCK").sum())
        avg_dos  = policy_f["dos"].replace(999, np.nan).mean()
        total_val = policy_f["inventory_value_vnd"].sum() / 1e9

        cards = '<div class="kpi-grid">'
        cards += kpi_card("Critical",  str(n_crit),   "below safety stock", "danger")
        cards += kpi_card("Reorder",   str(n_reorder),"needs replenishment", "warn")
        cards += kpi_card("OK",        str(n_ok),     "healthy", "accent3")
        cards += kpi_card("Overstock", str(n_over),   "excess inventory", "accent")
        cards += kpi_card("Avg DOS",   f"{avg_dos:.1f}d" if not np.isnan(avg_dos) else "—", "days of supply", "accent")
        cards += kpi_card("Inv Value", f"{total_val:.1f}B", "VND (filtered)", "accent2")
        cards += '</div>'
        st.markdown(cards, unsafe_allow_html=True)
    ...
    with col5:
        section("Policy Status by Warehouse")
        if not policy_f.empty:
            wh_status = policy_f.groupby(["warehouse","policy_status"]).size().reset_index(name="count")
            fig = px.bar(
                wh_status, x="warehouse", y="count", color="policy_status",
                color_discrete_map=STATUS_COLORS, barmode="stack",
            )
            fig.update_layout(**plotly_layout(height=280, xaxis_title="Warehouse", yaxis_title="SKU Count", showlegend=True))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No inventory data.")

    with col6:
        section("Inventory Value vs Safety Stock Coverage")
        if not policy_f.empty:
            fig = px.scatter(
                policy_f, x="safety_stock", y="inventory_value_vnd",
                color="policy_status", size="on_hand_units",
                hover_data=["sku","warehouse","dos","stockout_risk"],
                color_discrete_map=STATUS_COLORS,
            )
            fig.update_layout(**plotly_layout(
                height=280,
                xaxis_title="Safety Stock (units)",
                yaxis_title="Inventory Value (VND)",
            ))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No inventory data.")
    ...
    with col7:
        ...
        if not policy_f.empty:
            ...
            pivot = heat.pivot_table(index="warehouse", columns="abc_class", values="dos_plot", aggfunc="mean").fillna(0)
            ...
            fig.update_layout(**plotly_layout(height=300, xaxis_title="ABC Class", yaxis_title=""))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No inventory data.")

    with col8:
        section("EOQ vs Safety Stock by ABC Class")
        if not policy_f.empty:
            fig = px.scatter(
    ...
    if not policy_f.empty:
        ...
        if not top20.empty:
            ...
            fig.update_layout(**plotly_layout(barmode="overlay", height=320, xaxis_tickangle=-45, xaxis_title="", yaxis_title="Units"))
            st.plotly_chart(fig, use_container_width=True)
...
with tab4:
    ...
    with sf1:
        sup_opts = ["All"] + (sorted(plan_base["supplier_id"].unique().tolist()) if not plan_base.empty else [])
    ...
    with sf2:
        if not plan_base.empty and "expected_arrival" in plan_base.columns:
            ...
            sel_arrival = st.slider("Arrival Date Range", arr_min, arr_max, (arr_min, arr_max), key="sup_arrival")
        else:
            sel_arrival = (None, None)

    with sf3:
        min_order_val = st.number_input("Min Order Value (M VND)", min_value=0, max_value=500, value=0, step=10, key="sup_minval")
    ...
    with col9:
        ...
        if not plan_f.empty:
            wh_plan = (plan_f.groupby("warehouse")
                       ...
                       .reset_index().sort_values("total_value"))
            wh_plan["total_value_b"] = wh_plan["total_value"] / 1e9
            fig = go.Figure(go.Bar(
                x=wh_plan["total_value_b"], y=wh_plan["warehouse"],
                orientation="h",
                marker=dict(
                    color=wh_plan["total_value_b"],
                    colorscale=[[0,"#1e2736"],[1,"#ff6b35"]], showscale=False,
                ),
                text=wh_plan["total_value_b"].map(lambda x: f"{x:.2f}B"),
                textposition="outside", textfont=dict(size=10),
            ))
            fig.update_layout(**plotly_layout(height=280, xaxis_title="Order Value (B VND)", yaxis_title=""))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No supply data.")

    with col10:
        section("Orders by ABC Class × Status")
        if not plan_f.empty and "abc_class" in plan_f.columns and "policy_status" in plan_f.columns:
            abc_status = plan_f.groupby(["abc_class","policy_status"]).size().reset_index(name="count")
            fig = px.bar(
                abc_status, x="abc_class", y="count", color="policy_status",
                color_discrete_map=STATUS_COLORS, barmode="group",
            )
            fig.update_layout(**plotly_layout(height=280, xaxis_title="ABC Class", yaxis_title="Orders", showlegend=True))
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No supply data.")
    ...
    with col11:
        ...
        if not plan_f.empty and "supplier_id" in plan_f.columns:
    ...
    with col12:
        ...
        if not plan_f.empty and "dos_before_order" in plan_f.columns:
            fig = px.scatter(
