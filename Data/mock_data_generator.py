...
def generate_product_master(n_sku: int = N_SKU, seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(n_sku):
        ...
        # ABC classification based on cost
        if unit_cost >= 20_000:
            abc = "A"
...
def generate_lead_time(
    ...
    for _, row in product_master.iterrows():
        # Each SKU is sourced from 1–2 suppliers
...
def generate_sales_history(
    ...
    Output columns:
      date | sku | warehouse | sales_qty | sales_value_vnd
      | is_promotion_day | stockout_flag
    """
    r     = _rng(seed + 300)
    rows  = []
    ...
    # Pre-generate per-SKU demand parameters
    ...
    # Promotion day index (roughly 15% of days are promo)
    promo_days = set(r.choice(history_days, size=int(history_days * 0.15), replace=False).tolist())
    ...
    for day_idx, date in enumerate(dates):
        ...
        for wh in WAREHOUSES:
            ...
            for _, row in product_master.iterrows():
                ...
                # Simulate ~5% stockout probability
                stockout = bool(r.random() < 0.05)
                if stockout:
                    sales_qty = int(demand * float(r.uniform(0.0, 0.7)))
                else:
                    sales_qty = demand

                sales_value = sales_qty * row["unit_cost_vnd"]
                ...
                rows.append({
                    ...
                    "warehouse":         wh,
                    "sales_qty":         sales_qty,
                    "demand_qty":        demand,          # true demand (before stockout)
                    "sales_value_vnd":   sales_value,
...
def generate_inventory_snapshot(
    ...
    """
    Current on-hand inventory (latest snapshot).
    Output columns:
    ...
    for wh in WAREHOUSES:
        for _, row in product_master.iterrows():
            ...
            # Assign lot numbers
            n_lots = int(r.integers(1, 4))
            lot    = f"LOT-{r.integers(10000, 99999)}"
            expiry = snap_date + pd.Timedelta(days=int(r.integers(30, row["shelf_life_days"])))
...
def generate_promotion_calendar(
    ...
    """
    Planned promotions for forecast horizon.
    Output columns:
...
def generate_cost_parameters(
    ...
    """
    Cost inputs for inventory optimization.
    Output columns:
    ...
    for _, row in product_master.iterrows():
        # Higher-value items → higher holding cost
        h_cost = round(float(r.uniform(0.15, 0.30)), 3)

        # ABC-based service level target
        sl_target = {
...
# ══════════════════════════════════════════════
# MASTER RUNNER – generate all datasets
# ══════════════════════════════════════════════
...
if __name__ == "__main__":
    datasets = generate_all(seed=42, save_csv=True, output_dir="data/outputs")
